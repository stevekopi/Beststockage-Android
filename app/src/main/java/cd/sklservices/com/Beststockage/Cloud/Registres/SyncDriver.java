...
public class SyncDriver extends DaoDist implements AsyncResponse {

    private DriverRepository driverRepository;
    public SyncDriver(DriverRepository repo) {
            ...
            public void run (){
                try{
                    ...
                    if (output.contains("AddingDate")){
                        try {
                            Log.d("Assert","Synchronisation driver "+output);
                            String data=output;
                            if(data.contains("ï»¿"))
                                data= data.replace("ï»¿","");
                            ...
                            for (int k=0;k<instances.length();k++){
                                ...
                                Driver instance=new Driver(id,proprietaireId,name,adding_date,updated_date,sync_pos,pos);
                                driverRepository.ajout_sync(instance);

                            }
                        } catch (JSONException e) {
                            Log.d("Assert"," Conversion JSONArray get_drivers impossible ************************* "+e.toString());
                        }
    ...
    public void envoi(){
        try {
            ...
            {
                link=AddressFormatForGet("driver",LessMonth());
            }
            else
            {
                link=AddressFormatForGet("driver",LessDay());
            }

            accesDonnees.execute(link);
        }
        catch (Exception e){
            Log.d("Assert","Synchronisation drivers ********************"+e.toString());
