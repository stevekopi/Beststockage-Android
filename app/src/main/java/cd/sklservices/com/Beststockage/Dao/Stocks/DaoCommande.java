...
public interface DaoCommande {
    ...
    @Query("SELECT * FROM commande WHERE id LIKE  :Id")
    public Commande get(String Id) ;

    @Query("SELECT * FROM commande WHERE sync_pos!='3' and sync_pos!='4' ORDER BY date DESC ")
    public List<Commande> select_commande() ;

    @Query("SELECT * FROM commande WHERE sync_pos!='3' and sync_pos!='4'  ORDER BY date DESC ")
    public List<Commande> select_orderbydate_commande() ;

    @Query("SELECT * FROM commande WHERE  sync_pos!='3' and sync_pos!='4' and adding_user_id= :Id_user AND proprietaire_id= :Id_prorietaire AND fournisseur_id= :Id_fournisseur " +
    ...
    @Query("SELECT ifnull(sum(a.quantite),0) from approvisionnement as a inner join livraison as l on a.livraison_id=l.id " +
            ...
            " inner join commande as c on lc.commande_id=c.id WHERE c.id= :IdCommande and c.sync_pos!='3' and c.sync_pos!='4'")
    public List<String> getSommeQuantiteApprovisionner(String IdCommande) ;

    @Query("SELECT * FROM commande WHERE sync_pos = '0' and sync_pos='3' ORDER BY RANDOM() ")
